...
3. Checkout creates a **pending order**, then redirects to payment (Flitt or test simulator).
...
## Payments (Flitt — not LariPay SaaS)

Code: `lib/payments/`, docs: **`docs/PAYMENTS-FLITT.md`**. Tickets are **not** created until payment is confirmed.

| Mode | When | Flow |
|------|------|------|
| **Test** | No `FLITT_MERCHANT_ID` / `FLITT_SECRET_KEY`, or `SABAGIRO_PAYMENTS_TEST_MODE=true` | `/payment/test` → simulate Pay / Cancel |
| **Flitt** | Credentials on Vercel | Redirect to Flitt checkout → webhook `/api/webhooks/flitt` → `/payment/return` |

Env (production on Vercel):

- `SABAGIRO_PUBLIC_URL` or `APP_URL` — e.g. `https://www.sabagiro.ge`
- `FLITT_MERCHANT_ID`, `FLITT_SECRET_KEY`
- Optional: `FLITT_API_ORIGIN` (default `https://pay.flitt.com`)
